...
def build_xgb_model(**params):
    return XGBRegressor(objective="reg:squarederror", random_state=42, n_jobs=-1, **params)


def tune_xgb_model(X_train_data, y_train_data, n_iter=15):
...
def run_xgb_experiment(X_tr, X_val, title_prefix, plot_name):
    best_params, best_cv_rmse, tune_time = tune_xgb_model(X_tr, y_train)
...
result_xgb_raw = run_xgb_experiment(X_train_raw, X_val_raw, "XGBoost（PCA前）", "xgboost_before_pca_val_pred.png")
result_xgb_pca = run_xgb_experiment(X_train_pca, X_val_pca, "XGBoost（PCA后）", "xgboost_after_pca_val_pred.png")
compare_xgb_df = pd.DataFrame([result_xgb_raw, result_xgb_pca])
print("\n==============================")
print("XGBoost 验证集结果汇总")
print("==============================")
print(compare_xgb_df[["模型", "交叉验证最佳RMSE", "验证集RMSE", "验证集MAE", "验证集R2", "调参耗时(s)", "最终训练与验证耗时(s)", "总耗时(s)"]])
print("\n【PCA前详细结果】")
print(result_xgb_raw)
print("\n【PCA后详细结果】")
print(result_xgb_pca)
